...
function SignUp() {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      setLoading(false);
      const errorMessage = error.response?.data?.message || "Registration failed. Check network.";
  ...
  const handleGoogleLogin = async () => {
    ...
    if (!clerkInstance) {
      toast.error("Clerk is not initialized yet. Please refresh or wait a moment.");
      return;
    }

    try {
      // If already signed in to Clerk, sign out first to ensure we can choose a different account
      if (clerkInstance.user || clerkInstance.session) {
        await clerkInstance.signOut();
      }

      // Try to use the authenticateWithRedirect method on the best available target
      const target = clerkInstance.client?.signUp || clerkInstance;
      ...
      } else if (signUp && typeof signUp.sso === 'function') {
        // Fallback to sso method for Clerk v6 future resource
        await signUp.sso({
          strategy: "oauth_google",
          redirectUrl: `${window.location.origin}/sso-callback`,
    ...
    <div className="min-h-screen w-full flex flex-col lg:flex-row bg-white">
      ...
      <div className="flex-1 flex items-center justify-center px-5 py-10 sm:px-10 lg:px-16 xl:px-24 bg-white">
        <div className="w-full max-w-md">
          ...
          {/* Google */}
          <button
            className="w-full bg-white border border-stone-200 hover:border-stone-300 hover:bg-stone-50 active:scale-[0.98] text-stone-700 text-sm font-medium py-3.5 rounded-xl flex items-center justify-center gap-2.5 transition-all duration-200 cursor-pointer mb-6"
